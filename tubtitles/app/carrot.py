...
# Device name
DEVICE_NAME = "carrot"  # Define your device name here
...
def publish_process(shared_queue):
    ...
    start_server = websockets.serve(publish_messages, "0.0.0.0", 8756)
...
def language_receiver_process(shared_data):
    ...
    start_server = websockets.serve(receive_language_commands, "0.0.0.0", 8757)
    asyncio.get_event_loop().run_until_complete(start_server)
    asyncio.get_event_loop().run_forever()
